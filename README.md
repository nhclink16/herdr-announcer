...
Summaries are one spoken sentence generated by `codex exec` from the tail of
the agent's terminal output ("builder finished mid-cycle proration and all
fourteen invoice tests passed"). No Codex? Set `summary = "template"` for
instant fixed phrasing with no LLM call.
